...
def pick_gridcell_twist(loc_shift, grid_pattern, grid_ori, n, cur_scale):
...
def pick_gridcell_empty_edge(loc_shift, grid_cell_ensemble, scale_list, n, train_type):
    [yy, xx] = np.meshgrid(np.array(list(range(0, n))), np.array(list(range(0, n))))
    step_size_x = np.abs(loc_shift[0])
    step_size_y = np.abs(loc_shift[1])
    shift_x = xx - loc_shift[0]
...
def grid_score_60(corr, min_max=False):
    if min_max:
