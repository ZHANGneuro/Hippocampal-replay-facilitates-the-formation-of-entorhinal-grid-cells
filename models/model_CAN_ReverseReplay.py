...
# CAN model, reverse replay, no periodic boundary condition
...
# output folder name
task_type='original_CAN_reverse_replay'
...
for ith_start in list(range(0, len(when_to_pause_list))):
    ...
    for ith_move in list(range(0, when_to_pause)):
        ...
        for ith in list(range(0, spin_time)):
            grid_cell_ensemble = module_spin_class.spin_class_module(grid_cell_ensemble, w_matrix, n, thres_precent, 'single_neuron_4_scales')
        time_end = time.time()
        np.save(fig_export_path + '/' + str(task_type) + '_' + str(len(angles)) + 'dir_' + 'step' + str(iter_step) + '.npy', grid_cell_ensemble)
        ...
        grid_cell_ensemble = module_utility.no_periodic_boundary(loc_shift, grid_cell_ensemble, n, scale_list, 'multiscale')
    ...
    for ith_move in list(range(0, when_to_pause)):
        ...
        grid_cell_ensemble = module_utility.no_periodic_boundary(loc_shift, grid_cell_ensemble, n, scale_list,
                                                                     'multiscale')
        iter_step = iter_step + 1
