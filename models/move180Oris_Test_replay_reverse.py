...
for ith_start in list(range(0, len(when_to_pause_list))):
    ...
    for ith_move in list(range(0, when_to_pause)):
        ...
        grid_cell_ensemble = module_spin_class.spin_class_HD_module(grid_cell_ensemble=grid_cell_ensemble,
                                                                       w_matrix=w_matrix, n=n, spin_time=spin_time,
                                                                       thres_precent=thres_precent,
                                                                       train_type='single_neuron_4_scales')
        time_end = time.time()
        np.save(
            fig_export_path + '/' + str(task_type) + '_' + str(len(angle_offsets)) + 'dir_' + 'step' + str(iter_step) + '.npy',
            grid_cell_ensemble)
...
np.save(fig_export_path+'/size' + str(n) + '_' + str(task_type) + '_' + str(len(angle_offsets)) + 'dir_x.npy', np.array(coor_x))
np.save(fig_export_path+'/size' + str(n) + '_' + str(task_type) + '_' + str(len(angle_offsets)) + 'dir_y.npy', np.array(coor_y))
